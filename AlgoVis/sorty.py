...
class DrawInformation:
	...
	def __init__(self, width, height, lst):
		...
		pygame.display.set_caption("WELCOME TO ALGOVIS FUCKER")
		self.set_list(lst)
...
def draw(draw_info, algo_name, ascending):
	#Fill new window to clear previous frame
	draw_info.window.fill(draw_info.BACKGROUND_colour)

	#Blit text on top of canvas
	title = draw_info.LARGE_FONT.render(f"{algo_name} - {'Ascending' if ascending else 'Descending'}", 1, draw_info.AMAZON)
	...
	sorting1 = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort | S - Selection Sort", 1, draw_info.WHITE)
	draw_info.window.blit(sorting1, (draw_info.width/2 - sorting1.get_width()/2 , 65))

	sorting2 = draw_info.FONT.render("C - Cocktail Sort", 1, draw_info.WHITE)
	draw_info.window.blit(sorting2, (draw_info.width/2 - sorting2.get_width()/2 , 95))

	#Draw the list and update display
	draw_list(draw_info)
...
def draw_list(draw_info, colour_positions={}, clear_bg=False):
	...
	#loop through blocks and colour according to gradient
	for i, val in enumerate(lst):
		...
		#Check if colour should be green or red
